...
static enum ListType old_listType;

static BusStopListItem old_bus_stop_list_items[OLD_BUS_STOP_LIST_MAX_ITEMS];
static int old_bus_stop_list_num_of_items = 0;
static int old_bus_stop_list_active_item = -1;

static BusStopListItem* get_bus_stop_list_item_at_index(int index) {
	if (index < 0 || index >= OLD_BUS_STOP_LIST_MAX_ITEMS) {
		return NULL;
	} else {
		return &old_bus_stop_list_items[index];
	}
}

static void update_action_bar(void) {
	
	if (old_bus_stop_list_active_item == 0) {
	...
	if (old_bus_stop_list_active_item == old_bus_stop_list_num_of_items - 1) {
...
static void display_bus_stop_at_index(int index) {
	...
	old_bus_stop_list_active_item = index;
...
static void bus_stop_list_append(char *number, char *name, char *lines, int favorite) {
	...
	if (old_bus_stop_list_num_of_items == OLD_BUS_STOP_LIST_MAX_ITEMS) {
		return;
	}
	
	strcpy(old_bus_stop_list_items[old_bus_stop_list_num_of_items].number, number);
	strcpy(old_bus_stop_list_items[old_bus_stop_list_num_of_items].name, name);
	strcpy(old_bus_stop_list_items[old_bus_stop_list_num_of_items].lines, lines);
	old_bus_stop_list_items[old_bus_stop_list_num_of_items].favorite = favorite == 1;
	old_bus_stop_list_num_of_items++;
...
static void bus_stop_list_in_received_handler(DictionaryIterator *iter, void *context) {
	...
	if (no_bus_stops) {
		...
		if (old_listType == ListTypeFavorites) {
...
static void up_click_handler(ClickRecognizerRef recognizer, void *context) {
	...
	if (old_bus_stop_list_active_item > 0) {
		display_bus_stop_at_index(old_bus_stop_list_active_item - 1);
	}
}

static void select_click_handler(ClickRecognizerRef recognizer, void *context) {

	BusStopListItem *stopListItem = get_bus_stop_list_item_at_index(old_bus_stop_list_active_item);
	bus_stop_detail_show(stopListItem->number, stopListItem->name);
}

static void down_click_handler(ClickRecognizerRef recognizer, void *context) {

	if (old_bus_stop_list_active_item + 1 < old_bus_stop_list_num_of_items) {
		display_bus_stop_at_index(old_bus_stop_list_active_item + 1);
	}
}
...
static void add_remove_bus_stop_to_favorites() {
	...
	display_bus_stop_at_index(old_bus_stop_list_active_item);
...
static void bus_stop_window_unload(Window *window) {
	...
	old_bus_stop_list_num_of_items = 0;
	old_bus_stop_list_active_item = -1;
}
