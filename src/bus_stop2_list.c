...
#define OLD_BUS_STOP_LIST_MAX_ITEMS (20)
#define OLD_BUS_STOP_NAME_TEXT_LENGTH (60)
#define OLD_BUS_STOP_NUMBER_TEXT_LENGTH (6)
#define OLD_BUS_STOP_LINES_TEXT_LENGTH (20)
...
//        ScrollLayer *bus_scroll_layer;
	
	ActionBarLayer *action_bar_layer;
	
	TextLayer *feedback_text_layer;
	TextLayer *bus_stop_number_text_layer;
	TextLayer *bus_stop_name1_text_layer;
	TextLayer *bus_stop_name2_text_layer;
...
typedef struct {
	char number[OLD_BUS_STOP_NUMBER_TEXT_LENGTH];
	char name[OLD_BUS_STOP_NAME_TEXT_LENGTH];
	char lines[OLD_BUS_STOP_LINES_TEXT_LENGTH];
	bool favorite;
} BusStopListItem;

static enum ListType listType;

static BusStopListItem bus_stop_list_items[OLD_BUS_STOP_LIST_MAX_ITEMS];
...
static BusStopListItem* get_bus_stop_list_item_at_index(int index) {
	if (index < 0 || index >= OLD_BUS_STOP_LIST_MAX_ITEMS) {
		return NULL;
	} else {
...
static void display_bus_stop_at_index(int index) {
	...
	text_layer_set_text(ui.bus_stop_name1_text_layer, bus_stop_item->name);
//	text_layer_set_text(ui.bus_stop_name2_text_layer, bus_stop_item->name);
//	text_layer_set_text(ui.bus_stop_name_text_layer, bus_stop_item->lines);
	...
	//GRect nameRect = GRect(45, 2, 70, 20);
        // 50, 1, 70, 20
        // 50, 22, 70, 20
	//GSize size = graphics_text_layout_get_content_size(bus_stop_item->name, fonts_get_system_font(FONT_KEY_GOTHIC_14), nameRect, GTextOverflowModeTrailingEllipsis, GTextAlignmentCenter);
	...
	//layer_set_frame(text_layer_get_layer(ui.bus_stop_name_text_layer), nameRect);
        ...
        GRect nameRect = GRect(50, 1, 90, 31);
        GSize size = graphics_text_layout_get_content_size(bus_stop_item->name, fonts_get_system_font(FONT_KEY_GOTHIC_14), nameRect, GTextOverflowModeTrailingEllipsis, GTextAlignmentCenter);
	nameRect.origin.y = nameRect.origin.y + (nameRect.size.h - size.h) / 2;
	// nameRect.size.h = size.h + 2;
	layer_set_frame(text_layer_get_layer(ui.bus_stop_name1_text_layer), nameRect);
...
static void hide_bus_stop_detail_layers(bool hide) {
	...
	layer_set_hidden(text_layer_get_layer(ui.bus_stop_name1_text_layer), hide);
...
static void bus_stop_list_append(char *number, char *name, char *lines, int favorite) {
	...
	if (bus_stop_list_num_of_items == OLD_BUS_STOP_LIST_MAX_ITEMS) {
		return;
	}
...
static void bus_stop_window_load(Window *window) {
	...
	GRect bounds = layer_get_frame(window_layer);
        ...
        // - ui.bus_scroll_layer = scroll_layer_create(bounds);
	...
	// Number Text Layer
	ui.bus_stop_number_text_layer = text_layer_create(GRect(2, 0, 45, 40));
	text_layer_set_text_color(ui.bus_stop_number_text_layer, GColorBlack);
	text_layer_set_font(ui.bus_stop_number_text_layer, fonts_get_system_font(FONT_KEY_GOTHIC_28_BOLD));
	text_layer_set_text_alignment(ui.bus_stop_number_text_layer, GTextAlignmentCenter);
	layer_add_child(window_layer, text_layer_get_layer(ui.bus_stop_number_text_layer));
	
	// Name1 Text Label
	ui.bus_stop_name1_text_layer = text_layer_create(GRect(50, 1, 90, 31));
	text_layer_set_text_color(ui.bus_stop_name1_text_layer, GColorBlack);
	text_layer_set_font(ui.bus_stop_name1_text_layer, fonts_get_system_font(FONT_KEY_GOTHIC_14));
	text_layer_set_text_alignment(ui.bus_stop_name1_text_layer, GTextAlignmentCenter);
	layer_add_child(window_layer, text_layer_get_layer(ui.bus_stop_name1_text_layer));
        
        // Name2 Text Label
	ui.bus_stop_name2_text_layer = text_layer_create(GRect(50, 32, 70, 15));
	text_layer_set_text_color(ui.bus_stop_name2_text_layer, GColorBlack);
	text_layer_set_font(ui.bus_stop_name2_text_layer, fonts_get_system_font(FONT_KEY_GOTHIC_14));
	text_layer_set_text_alignment(ui.bus_stop_name2_text_layer, GTextAlignmentCenter);
	layer_add_child(window_layer, text_layer_get_layer(ui.bus_stop_name2_text_layer));
	...
	ui.bus_image_layer = bitmap_layer_create(GRect(5, 112, 30, 28));
	...
	ui.favorite_image_layer = bitmap_layer_create(GRect(100, 50, 14, 14));
    ...
    action_bar_layer_set_background_color(ui.action_bar_layer, GColorClear);
    //action_bar_layer_set_icon(ui.action_bar_layer, BUTTON_ID_UP, ui.previous_image);
...
static void bus_stop_window_unload(Window *window) {
	...
	text_layer_destroy(ui.bus_stop_name1_text_layer);
...
void bus_stop2_list_show_favorites(void) {
	
	load_view_for_bus_stops_type(ListTypeFavorites);
}

void bus_stop2_list_show_near(void) {
	
	load_view_for_bus_stops_type(ListTypeNear);
}

void bus_stop2_list_init(void) {
...
void bus_stop2_list_deinit(void) {
